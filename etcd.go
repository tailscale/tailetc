...
type Options struct {
	...
	// before Commit returns on that Tx but after the etcd server has
...
func (opts Options) fillDefaults() Options {
	...
	if opts.CloneFunc == nil {
		opts.CloneFunc = func(dst interface{}, key string, value interface{}) error {
			...
			reflect.ValueOf(dst).Elem().Set(reflect.ValueOf(cloned))
			return nil
		}
	}
	return opts
...
// Get gets a key-value from the etcd cache into value.
//
...
func (db *DB) watchResult(data []byte) error {
	...
	for _, newkv := range newkvs {
		...
		if exists && newkv.kvrev.modRev <= kv.modRev {
			// Value has just been updated by a Tx, keep newer value.
			continue
		}
		if db.opts.WatchFunc != nil {
			cloned, err := db.clone(newkv.key, newkv.kvrev.value)
			if err != nil {
...
func (tx *Tx) get(key string) (bool, kvrev, error) {
	if !strings.HasPrefix(key, tx.db.opts.KeyPrefix) {
		return false, kvrev{}, fmt.Errorf("key does not use prefix %s", tx.db.opts.KeyPrefix)
	}
	if tx.err != nil {
		return false, kvrev{}, tx.err
	}
